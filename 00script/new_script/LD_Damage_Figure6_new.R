...
boots<-read_csv("..\\01data\\04scenarios\\Bootstrap.csv")
...
globals<-data.frame()
for (i in c(1:1000)) {
  damage_col<- paste0("damage", i)
  loss_sub <- subset(loss, select = c("region", "year", damage_col))
  gdppc1<-left_join(gdppc_panel,loss_sub,by=c("region","year"))
  colnames(gdppc1)[6]<-"damage"
  ...
  global$model<-damage_col
  globals<-bind_rows(globals,global)
}

loss_summary <- globals %>%
  group_by(year) %>%
  summarise(
    mean = median(damage, na.rm = TRUE),
    sd = sd(damage, na.rm = TRUE),
    q10 = quantile(damage, 0.10, na.rm = TRUE),
    q90 = quantile(damage, 0.90, na.rm = TRUE),
  )
write_excel_csv(loss_summary,file = "..\\02table\\loss_panel_ssp245_summary.csv")

ggplot(data = loss_summary) +
  # 绘制95%置信区间
...
for (i in c(1:1000)) {
  ...
  loss_sub <- subset(loss, select = c("region", "year", damage_col))
  gdppc1<-left_join(gdppc_panel,loss_sub,by=c("region","year"))
  colnames(gdppc1)[7]<-"damage"
  gdppc1$loss<-gdppc1$gdppc*gdppc1$damage*gdppc1$pop
  continent<-gdppc1%>%
    group_by(continent,year) %>%
    summarise(gdp=sum(gdp,na.rm = T),loss=sum(loss,na.rm = T))
  continent$damage<-continent$loss/continent$gdp
  continent$model<-damage_col
  continents<-bind_rows(continents,continent)
}

loss_summary <- continents %>%
  group_by(continent,year) %>%
  summarise(
    mean = median(damage, na.rm = TRUE),
    sd = sd(damage, na.rm = TRUE),
...
save(loss, file = "..\\01data\\04scenarios\\bootstrap_ld_ssp224_loss_1000.RData")
load("..\\01data\\04scenarios\\bootstrap_ld_ssp224_loss_1000.RData")
...
for (i in c(1:1000)) {
  damage_col<- paste0("damage", i)
  loss_sub <- subset(loss, select = c("region", "period", damage_col))
  gdppc1<-left_join(gdppc_ld,loss_sub,by=c("region","period"))
  colnames(gdppc1)[6]<-"damage"
  gdppc1$loss<-gdppc1$gdppc*gdppc1$damage*gdppc1$pop
  global<-gdppc1%>%
    group_by(period) %>%
...
loss_summary$period<-2020+5*(loss_summary$period-1)
loss_summary[is.na(loss_summary)]<-0
ggplot(data = loss_summary) +
  # 绘制95%置信区间
  geom_ribbon(aes(x = period, ymin =q10, ymax =q90), 
              fill = rgb(252/255, 141/255, 98/255), alpha = 0.3) +
  # 绘制平均值的红色折线
...
######
loss_region<-filter(loss,period==17)
loss_region<-loss_region[,-2]
loss_region<-tidyr::gather(loss_region,damage,value, -region)
loss_region<-loss_region %>%
  group_by(region) %>%
...
a<-ggplot(data = map) +
  ...
  scale_fill_distiller(palette = "RdYlBu",
                       direction=1,
                       limits = c(-0.301, 0.301),                # 设置填充颜色的范围
                       oob = scales::squish,
                       name = "Percentage Change in GDP per capita" )+
  theme_minimal() +
  labs(fill="")+
