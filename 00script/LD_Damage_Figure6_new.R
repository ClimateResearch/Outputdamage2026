...
boots<-read_csv("..\\01data\\04scenarios\\Bootstrap5.csv")
...
loss_list <- lapply(1:1000, function(i) {
  ...
  # 设置参数
  thresholds <- 25200
  alphas <- c(selc_boot$T2_panel0, selc_boot$T2_panel1)
  betas <- c(selc_boot$T_panel0, selc_boot$T_panel1)
  
  gammas <- c(selc_boot$dT2_panel0, selc_boot$dT2_panel1)
  deltas <- c(selc_boot$dT_panel0, selc_boot$dT_panel1)
...
loss_list <- lapply(1:1000, function(i) {
  ...
  # 设置参数
  thresholds <- 25000
  lalphas <- c(selc_boot$lT2_ld0, selc_boot$lT2_ld1)
  lbetas <- c(selc_boot$lT_ld0, selc_boot$lT_ld1)
  alphas <- c(selc_boot$T2_ld0, 0)
  betas <- c(selc_boot$T_ld0, 0)
  
  gammas <- c(selc_boot$dT2_ld0, selc_boot$dT2_ld1)
  deltas <- c(selc_boot$dT_ld0, selc_boot$dT_ld1)
...
print(combined_plot )
 ...
 ggsave(
  filename = "..\\03figure\\Figure6_245.pdf")
